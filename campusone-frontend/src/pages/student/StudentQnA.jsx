...
const threads = [
  ...
  },
  {
    ...
    content: 'The midterm will cover chapters 1-5: Algorithm Analysis, Divide & Conquer, Greedy Algorithms, Dynamic Programming basics, and Graph fundamentals. Format: 3 theory questions (30 marks) + 2 coding problems (20 marks). Duration: 2 hours.',
    upvotes: 15,
    replies: 6,
    pinned: true,
    resolved: false,
  ...
  },
  {
    ...
    replyList: [
      ...
      { id: 2, author: 'Dr. Sarah Khan', authorRole: 'teacher', content: 'Good answer! A classic example: input validation — you want to ask the user at least once before checking.', createdAt: '2026-02-14T15:00:00', upvotes: 3 },
    ],
  },
  {
    id: 4,
    ...
    replyList: [
      { id: 1, author: 'Ayesha Siddiqui', authorRole: 'student', content: 'MIT OpenCourseWare has excellent lectures on this topic. Also check out "Big-O Cheat Sheet" website.', createdAt: '2026-02-13T16:30:00', upvotes: 7 },
      { id: 2, author: 'Dr. Ali Raza', authorRole: 'teacher', content: 'I recommend watching Abdul Bari\'s YouTube channel for algorithm analysis. Very intuitive explanations.', createdAt: '2026-02-13T17:00:00', upvotes: 10 },
    ],
  },
  {
    id: 5,
  ...
  {
    id: 6,
    title: 'IEEE citation format for technical reports?',
    courseCode: 'ENG201',
    ...
    content: 'I\'m confused about citing technical reports in IEEE format. Is it different from citing conference papers?',
    upvotes: 5,
    replies: 2,
    pinned: false,
    resolved: true,
    tags: ['ieee', 'citation'],
    replyList: [
      { id: 1, author: 'Ms. Rabia Tariq', authorRole: 'teacher', content: 'Yes, for technical reports the format is: Author, "Title," Company/Org, Report Number, Date. Check the IEEE reference guide I shared last week.', createdAt: '2026-02-11T10:00:00', upvotes: 4 },
    ],
  },
];
...
const courses = [
  ...
  { code: 'ENG201', name: 'Technical Writing' },
];
...
const StudentQnA = () => {
  ...
  const myPosts = threads.filter(t => t.isOwnPost).length;
  const totalReplies = threads.reduce((s, t) => s + t.replies, 0);

  const stats = [
    { icon: MessageSquare, label: 'All Threads', value: threads.length, color: '#3b82f6' },
    { icon: User, label: 'My Posts', value: myPosts, color: '#8b5cf6' },
    { icon: MessageCircle, label: 'Total Replies', value: totalReplies, color: '#06b6d4' },
    { icon: CheckCircle2, label: 'Resolved', value: threads.filter(t => t.resolved).length, color: '#10b981' },
  ];
    ...
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6 max-sm:flex-col max-sm:items-start max-sm:gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 m-0 mt-1">Ask questions, share knowledge, and discuss with classmates</p>
        </div>
        <button
      ...
      {showCreate && (
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-5">
          <h3 className="text-base font-semibold text-slate-800 m-0 mb-4">Ask a Question</h3>
          <div className="space-y-3">
            ...
            <input className="w-full py-2.5 px-3.5 border border-gray-200 rounded-lg text-[0.95rem] focus:outline-none focus:border-blue-500" placeholder="Question title..." />
            <textarea rows={3} className="w-full py-2.5 px-3.5 border border-gray-200 rounded-lg text-[0.95rem] focus:outline-none focus:border-blue-500 resize-none" placeholder="Describe your question in detail..." />
            <div className="flex justify-end gap-2">
              ...
              <button className="py-2 px-4 border-none rounded-lg text-sm font-medium cursor-pointer bg-blue-600 text-white hover:bg-blue-700">Post Question</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        {sorted.map((t) => {
          ...
          return (
            <div key={t.id} className={`bg-white rounded-2xl shadow-sm overflow-hidden transition-all ${t.pinned ? 'ring-1 ring-purple-200' : ''} ${t.isOwnPost ? 'border-l-4 border-l-blue-400' : ''}`}>
              <div
      ...
      {sorted.length === 0 && (
        <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-slate-500 m-0">Be the first to ask a question!</p>
        </div>
      )}
    </div>
  );
